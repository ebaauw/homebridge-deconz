...
class DeconzAccessory extends homebridgeLib.AccessoryDelegate {
  ...
  updateService (service, rpath, body) {
    if (service === this.consumptionService) {
      DeconzService.Consumption.update(service, rpath, body)
    } else if (service === this.powerService) {
