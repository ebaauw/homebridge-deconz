...
const { toBool, toInt, toObject, toString } = homebridgeLib.OptionParser
const { parseUniqueid } = Deconz.ApiClient
...
class Gateway extends homebridgeLib.AccessoryDelegate {
  ...
  analyseResource (rtype, rid, body, logUnsupported) {
    ...
    toBool('logUnsupported', logUnsupported)
    const { id, type } = attrs
    const { category, serviceName } = type
    if (id === this.id || serviceName == null) {
    ...
    if (this.deviceById[id] == null) {
      this.deviceById[id] = new Deconz.Device(rtype, rid, body, attrs)
  ...
  /** Dervice the resource attributes for a resource.
    *
    * @param {string} rtype - The resource type of the resource:
    * `groups`, `lights`, or `sensors`.
    * @param {integer} rid - The resource ID of the resource.
    * @param {object} body - The body of the resource.
    * @returns {Deconz.ResourceAttributes} - The derived resource
    * attributes.
    */
  resourceAttrs (rtype, rid, body) {
    toString('rtype', rtype, true)
    ...
    if (rtype === 'lights' || (rtype === 'sensors' && body.type.startsWith('Z'))) {
      ...
      return new Deconz.ResourceAttributes(
        mac,
  ...
  /** Derive the type attributes for a `lights` resource.
    *
    * @params {string} type - The `type` in the resource body.
    * @return {Deconz.TypeAttributes} - The derived type attributes.
    */
  lightsTypeAttributes (type) {
    toString('type', type, true)
    const Cats = this.Accessory.Categories
    switch (type) {
      case 'Color dimmable light':
        return new Deconz.TypeAttributes(Cats.LIGHTBULB, 'LightBulb', 4)
      case 'Color light':
        return new Deconz.TypeAttributes(Cats.LIGHTBULB, 'LightBulb', 3)
      case 'Color temperature light':
        return new Deconz.TypeAttributes(Cats.LIGHTBULB, 'LightBulb', 2)
      case 'Dimmable light':
        return new Deconz.TypeAttributes(Cats.LIGHTBULB, 'LightBulb', 1)
      case 'Dimmable plug-in unit':
        return new Deconz.TypeAttributes(Cats.OUTLET, 'LightBulb')
      case 'Extended color light':
        return new Deconz.TypeAttributes(Cats.LIGHTBULB, 'LightBulb', 5)
      // case 'Consumption awareness device':
      //   return new Deconz.TypeAttributes(Cats.OTHER)
      case 'Dimmer switch':
        return new Deconz.TypeAttributes(Cats.LIGHTBULB, 'LightBulb')
      case 'Level control switch':
        return new Deconz.TypeAttributes(Cats.LIGHTBULB, 'LightBulb')
      // case 'Level controllable output':
      //   return new Deconz.TypeAttributes(Cats.OTHER)
      // case 'Door Lock':
      //   return new Deconz.TypeAttributes(Cats.DOOR_LOCK)
      // case 'Door Lock Unit':
      //   return new Deconz.TypeAttributes(Cats.DOOR_LOCK)
      // case 'Fan':
      //   return new Deconz.TypeAttributes(Cats.FAN)
      case 'On/Off light switch':
        return new Deconz.TypeAttributes(Cats.SWITCH, 'Outlet')
      case 'On/Off light':
        return new Deconz.TypeAttributes(Cats.LIGHTBULB, 'Outlet')
      case 'On/Off output':
        return new Deconz.TypeAttributes(Cats.OUTLET, 'Outlet')
      case 'On/Off plug-in unit':
        return new Deconz.TypeAttributes(Cats.OUTLET, 'Outlet')
      case 'Smart plug':
        return new Deconz.TypeAttributes(Cats.OUTLET, 'Outlet')
      case 'Configuration tool':
        return new Deconz.TypeAttributes(Cats.BRIDGE)
      case 'Range extender':
        return new Deconz.TypeAttributes(Cats.RANGE_EXTENDER)
      case 'Warning device':
        return new Deconz.TypeAttributes(Cats.SECURITY_SYSTEM, 'WarningDevice')
      case 'Window covering controller':
        return new Deconz.TypeAttributes(Cats.WINDOW_COVERING, 'WindowCovering')
      case 'Window covering device':
        return new Deconz.TypeAttributes(Cats.WINDOW_COVERING, 'WindowCovering')
      default:
        return new Deconz.TypeAttributes()
    }
  }

  /** Derive the type attributes for a `sensors` resource.
    *
    * @params {string} type - The `type` in the resource body.
    * @return {Deconz.TypeAttributes} - The derived type attributes.
    */
  sensorsTypeAttributes (type) {
    ...
    switch (type) {
      ...
      case 'CLIPAirPurifier':
        return new Deconz.TypeAttributes(Cats.AIR_PURIFIER)
      case 'ZHAAirQuality':
      case 'CLIPAirQuality':
        return new Deconz.TypeAttributes(Cats.SENSOR, 'AirQuality', 3)
      case 'ZHAAlarm':
      case 'CLIPAlarm':
        return new Deconz.TypeAttributes(Cats.SECURITY_SYSTEM, 'Alarm')
      // case 'ZHAAncillaryControl':
      // case 'CLIPAncillaryControl':
      //   return new Deconz.TypeAttributes(Cats.SECURITY_SYSTEM)
      case 'ZHABattery':
      case 'CLIPBattery':
        return new Deconz.TypeAttributes(Cats.SENSOR, 'Battery')
      case 'ZHACarbonMonoxide':
      case 'CLIPCarbonMonoxide':
        return new Deconz.TypeAttributes(Cats.SENSOR, 'CarbonMonoxide')
      case 'ZHAConsumption':
      case 'CLIPConsumption':
        return new Deconz.TypeAttributes(Cats.SENSOR, 'Consumption')
      // case 'ZHADoorLock':
      // case 'CLIPDoorLock':
      //   return new Deconz.TypeAttributes(Cats.DOOR_LOCK)
      case 'Daylight':
        return new Deconz.TypeAttributes(Cats.SENSOR, 'Daylight')
      case 'ZHAFire':
      case 'CLIPFire':
        return new Deconz.TypeAttributes(Cats.SENSOR, 'Fire')
      case 'CLIPGenericFlag':
        return new Deconz.TypeAttributes(Cats.SWITCH, 'Flag')
      case 'CLIPGenericStatus':
        return new Deconz.TypeAttributes(Cats.OTHER, 'Status')
      case 'ZHAHumidity':
      case 'CLIPHumidity':
        return new Deconz.TypeAttributes(Cats.SENSOR, 'Humidity', 4)
      case 'ZHALightLevel':
      case 'CLIPLightLevel':
        return new Deconz.TypeAttributes(Cats.SENSOR, 'LightLevel', 6)
      // case 'ZHAMoisture':
      // case 'CLIPMoisture':
      //   return new Deconz.TypeAttributes(Cats.SENSOR)
      case 'ZHAOpenClose':
      case 'CLIPOpenClose':
        return new Deconz.TypeAttributes(Cats.SENSOR, 'OpenClose', 8)
      case 'ZHAPower':
      case 'CLIPPower':
        return new Deconz.TypeAttributes(Cats.SENSOR, 'Power', 1)
      case 'ZHAPresence':
      case 'CLIPPresence':
        return new Deconz.TypeAttributes(Cats.SENSOR, 'Presence', 7)
      case 'ZHAPressure':
      case 'CLIPPressure':
        return new Deconz.TypeAttributes(Cats.SENSOR, 'Pressure', 3)
      case 'ZHASpectral':
        return new Deconz.TypeAttributes(Cats.SENSOR)
      case 'ZGPSwitch':
      case 'ZHASwitch':
        return new Deconz.TypeAttributes(Cats.PROGRAMMABLE_SWITCH, 'Switch')
      case 'CLIPSwitch':
        return new Deconz.TypeAttributes(Cats.PROGRAMMABLE_SWITCH)
      case 'ZHATemperature':
      case 'CLIPTemperature':
        return new Deconz.TypeAttributes(Cats.SENSOR, 'Temperature', 5)
      case 'ZHAThermostat':
      case 'CLIPThermostat':
        return new Deconz.TypeAttributes(Cats.THERMOSTAT, 'Thermostat')
      case 'ZHATime':
      case 'CLIPTime':
        return new Deconz.TypeAttributes(Cats.OTHER)
      case 'ZHAVibration':
      case 'CLIPVibration':
        return new Deconz.TypeAttributes(Cats.SENSOR, 'Vibration')
      case 'ZHAWater':
      case 'CLIPWater':
        return new Deconz.TypeAttributes(Cats.SENSOR, 'Water')
      default:
        return new Deconz.TypeAttributes()
    }
  }
