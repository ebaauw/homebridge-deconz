...
class Gateway extends AccessoryDelegate {
  ...
  constructor (platform, params) {
    ...
    this.addPropertyDelegate({
      ...
      if (this.wsClient != null) {
        this.wsClient.host = this.wsClient.host.split(':')[0] +
          ':' + this.values.wsPort
      }
    })
  ...
  update (config) {
    ...
    this.values.wsPort = config.websocketport_wss ?? config.websocketport
  ...
  createClient () {
    ...
    this.client = new ApiClient({
      ...
      https: this.context.config.https,
      fingerprint: this.context.config.fingerprint,
  ...
  /** Create {@link DeconzAccessory.Gateway#wsclient}.
    */
  createWsClient () {
    const port = this.context.fullState?.config.websocketport_wss ??
      this.context.fullState?.config.websocketport
    const options = {
      wss: this.context.fullState.config.websocketport_wss != null,
      fingerprint: this.context.config.fingerprint,
