...
class Gateway extends AccessoryDelegate {
   ...
   */
  analyseBlacklist (logUnsupported = false) {
    ...
    for (const key in this.context.fullState.resourcelinks) {
      ...
      if (
        link.name === 'homebridge-deconz' && link.links &&
        link?.description.toLowerCase() === 'blacklist'
      ) {
        this.debug('/resourcelinks/%d: %d blacklist entries', key, link.links.length)
        for (const resource of link.links) {
          const rtype = resource.split('/')[1]
          const rid = resource.split('/')[2]
          if (this.blacklist[rtype] == null) {
            warn('/resourcelinks/%d: %s: ignoring unsupported resource', key, resource)
            continue
          }
          this.blacklist[rtype][rid] = true
        }
      }
    }
  }
  ...
  async analyseFullState (fullState, params = {}) {
    ...
    this.analyseBlacklist(params.logUnsupported)
