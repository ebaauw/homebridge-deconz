...
class Gateway extends homebridgeLib.AccessoryDelegate {
  ...
  constructor (platform, params) {
    ...
    /** Map of Accessory delegates by id for the gayeway.
      * @type {Object<string, DeconzAccessory>}
      */
    this.accessoryById = {}
    this.accessoryById[this.id] = this.gateway

    /** Map of Accessory delegates by rpath for the gayeway.
      * @type {Object<string, DeconzAccessory>}
      */
    this.accessoryByRpath = {}
    this.accessoryByRpath['/config'] = this.gateway

    /** Map of services to un-blacklist a gateway device.
      * @type {Object<string, DeconzService.Device>}
      */
    this.serviceById = {}
  ...
  /** List of ps paths of gateway resources currently monitored.
    * @type {string[]}
    */
  ...
  /** List of IDs of blacklisted devices.
    * @type {string[]}
    */
  get blacklist () { return Object.keys(this.serviceById).sort() }
  ...
  async createUser (retry = 0) {
    ...
    try {
      ...
      if (
        ...
        return this.createUser(retry + 1)
      }
    }
  ...
  /** (Re-/un-)expose the accessory for the device.
  ...
  /** Add a service to the gateway accessory to un-blacklist the device.
    * @params {string} id - The device ID.
    * @return {DeconzService.Device} - The service delegate.
    */
  addService (id) {
    if (id === this.id) {
      return
    ...
    if (this.serviceById[id] == null) {
      ...
      this.serviceById[id] = new DeconzService.Device(this, {
        name: body.name,
        subtype: id,
        resource: rpath,
        expose: false
