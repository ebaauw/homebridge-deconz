...
class Gateway extends homebridgeLib.AccessoryDelegate {
  ...
  /** List of resource paths of gateway resources currently monitored.
    * @type {string[]}
    */
  get accessoryRpaths () { return Object.keys(this.accessoryByRpath).sort() }
  ...
  identify () {
    this.log(
      '%s %s gateway v%s (%d devices, %d resources)',
      this.values.manufacturer, this.values.model, this.values.software,
      this.deviceIds.length, this.accessoryRpaths.length
    )
    this.debug(
      '%d resouces: %j', this.resourceRpaths.length, this.resourceRpaths
    )
    this.debug(
    ...
    this.debug(
      'monitoring %d resources: %j', this.accessoryRpaths.length, this.accessoryRpaths
    )
    this.debug(
  ...
  addAccessory (id) {
    ...
    if (this.accessoryById[id] == null) {
      ...
      this.accessoryById[id] = new DeconzAccessory.Device(this, device)
      this.monitorResources(this.accessoryById[id], true)
    }
  ...
  addService (id) {
    ...
    if (this.serviceById[id] == null) {
      const { resource } = this.deviceById[id]
      const { body, rpath } = resource
  ...
  /** List of IDs of all devices.
    * @type {string[]}
    */
  get deviceIds () { return Object.keys(this.deviceById || {}) }

  /** List of resource paths of all resources.
    * @type {string[]}
    */
  get resourceRpaths () { return Object.keys(this.resourceByRpath || {}).sort() }

  deviceRpaths (rtype) {
    return Object.keys(this[rtype + 'DeviceByRid']).sort().map((rid) => {
      return this[rtype + 'DeviceByRid'][rid].resource.rpath
    })
  }
