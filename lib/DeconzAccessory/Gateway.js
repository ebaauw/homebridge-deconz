...
class Gateway extends homebridgeLib.AccessoryDelegate {
  ...
  _accessoryToMap (accessory) {
    const settings = accessory.settingsService.values
    return {
      id: accessory.id,
      name: accessory.name,
  ...
  _deviceToMap (device) {
    const resource = device.resourceBySubtype[device.primary]
    return {
      id: device.id,
      zigbee: device.zigbee,
      rtype: resource.rtype,
      manufacturername: resource.manufacturer,
      modelid: resource.model,
      name: resource.body.name,
      rpaths: resource.rpaths,
    ...
    if (a[0] === 'accessories') {
      if (a.length === 1) {
        ...
        for (const id of Object.keys(this.accessoryById).sort()) {
          body[id] = this._accessoryToMap(this.accessoryById[id])
        }
      ...
      if (a.length === 2) {
        ...
        return { status: 200, body: this._accessoryToMap(this.accessoryById[id]) }
      }
    }
    if (a[0] === 'devices') {
      if (a.length === 1) {
        ...
        for (const id of Object.keys(this.deviceById).sort()) {
          body[id] = this._deviceToMap(this.deviceById[id])
        }
        return { status: 200, body }
      }
      if (a.length === 2) {
        ...
        return { status: 200, body: this._deviceToMap(this.deviceById[id]) }
      }
    }
  ...
  async onUiPut (a, body) {
    ...
    if (a[0] === 'accessories') {
      if (a.length === 2) {
        ...
        if (body.logLevel != null) {
          this.accessoryById[id].settingsService.values.logLevel = body.logLevel
        }
    ...
    return { status: 403 } // Not Implented
  }
