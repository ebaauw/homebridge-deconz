...
class Gateway extends homebridgeLib.AccessoryDelegate {
  ...
  constructor (platform, params) {
    ...
    for (const rtype of rtypes) {
      const service = new DeconzService.DeviceSettings(this, {
        name: rtype[0].toUpperCase() + rtype.slice(1),
        subtype: rtype,
        resource: '/' + rtype,
        expose: false
      })
      service.characteristicDelegate('expose')
        .on('didSet', (value) => {
          if (value) {
            this.context.rtypes.push(rtype)
          } else {
            this.context.rtypes.splice(this.context.rtypes.indexOf(rtype), 1)
          }
        })
      this[rtype + 'Service'] = service
    }
  ...
  async poll () {
    ...
    try {
      this.context.fullState = {}
      this.context.fullState.config = await this.client.get('/config')
      this.check(this.context.fullState.config)
      this.context.fullState.lights = await this.client.get('/lights')
      this.context.fullState.sensors = await this.client.get('/sensors')
      if (this.context.rtypes.includes('groups')) {
  ...
  analyseFullState () {
    ...
    for (const id in this.accessoryById) {
      if (
        ...
        !this.context.rtypes.includes(this.deviceById[id].resource.rtype)
      ) {
        this.deleteAccessory(id)
        this.deleteService(id)
        changed = true
    ...
    for (const rtype of this.context.rtypes) {
      ...
      for (const rid of rids) {
        ...
        if (this.accessoryById[id] == null && this.context.blacklist[id] == null) {
          this.addAccessory(id)
          changed = true
        }
      }
