...
class Gateway extends homebridgeLib.AccessoryDelegate {
  ...
  constructor (platform, params) {
    ...
    for (const rtype of rtypes) {
      const service = new DeconzService.DeviceSettings(this, {
        name: rtype[0].toUpperCase() + rtype.slice(1) + ' Settings',
        subtype: rtype,
        resource: '/' + rtype,
        expose: false
      })
      this.rtypeService[rtype] = service
      service.characteristicDelegate('expose')
        .on('didSet', async (value) => {
          try {
            if (value) {
              this.rtypes.push(rtype)
            } else {
              this.rtypes.splice(this.rtypes.indexOf(rtype), 1)
            }
            await this.poll()
          } catch (error) { this.warn(error) }
        })
      if (service.values.expose) {
        this.rtypes.push(rtype)
      }
    }
  ...
  /** Log debug messages.
    */
  identify () {
    const accessories = Object.keys(this.accessoryById).sort()
    const devices = Object.keys(this.deviceById).sort()
    const resources = Object.keys(this.resourceByRpath).sort()
    const exposeErrors = Object.keys(this.exposeErrorById).sort()
    const usedResources = Object.keys(this.accessoryByRpath).sort()
    const blacklist = Object.keys(this.context.blacklist).sort()

    this.log(
      '%s %s gateway v%s (exposing %d accessories for %d devices, %d resources)',
      this.values.manufacturer, this.values.model, this.values.software,
      accessories.length, devices.length, resources.length
    )
    this.debug('%d resouces: %j', resources.length, resources)
    this.debug('%d devices: %j', devices.length, devices)
    this.debug('exposing %d accessories: %j', accessories, accessories)
    this.debug('%d expose errors: %j', exposeErrors.length, exposeErrors)
    this.debug('monitoring %d resources: %j', usedResources.length, usedResources)
    this.debug('blacklist: %d devices: %j', blacklist.length, blacklist)
  }
  ...
  async init (beat) {
    try {
      ...
      if (this.context.fullState != null) {
        // Rebuild deviceById and resourceByRpath from cached bridge state,
  ...
  async connect (retry = 0) {
    ...
    try {
      ...
      for (const id in this.exposeErrorById) {
        this.log('%s: reviving after %s', id, this.exposeErrorById[id])
  ...
  async reset () {
    ...
    try {
      ...
      for (const rtype of rtypes) {
        this.rtypeService[rtype].values = false
      }
    } catch (error) { this.error(error) }
  }
  ...
  addAccessory (id) {
    ...
    if (this.accessoryById[id] == null) {
      ...
      this.deleteService(id)
      const accessory = new DeconzAccessory.Device(this, device)
  ...
  /** Delete the accessory for the device.
    * @params {string} id - The device ID.
    */
  deleteAccessory (id, autoRevive = true) {
    if (id === this.id) {
      throw new RangeError(`${id}: gateway ID`)
    }
    if (this.accessoryById[id] != null) {
  ...
  async poll () {
    ...
    const fullState = {}
    try {
      fullState.config = await this.client.get('/config')
      if (fullState.config.UTC == null) {
        this.service.values.expose = false
        this.service.values.username = null
        await this.wsClient.close()
        return
      ...
      if (
        this.rtypeService.lights.values.expose ||
        this.rtypeService.sensors.values.expose
      ) {
        fullState.lights = await this.client.get('/lights')
        fullState.sensors = await this.client.get('/sensors')
      }
      if (this.rtypeService.groups.values.expose) {
        fullState.groups = await this.client.get('/groups')
        fullState.groups[0] = await this.client.get('/groups/0')
      }
      this.analyseFullState(fullState)
      // if (this.rtypes.includes('schedules')) {
      //   fullState.schedules = await this.client.get('/schedules')
      // }
      // this.analyseSchedules()
  ...
  analyseFullState () {
    this.vdebug('analysing full state')
    ...
    const nDevices = Object.keys(this.deviceById).length
    const nResources = Object.keys(this.resourceByRpath).length
    this.debug('%d devices, %d resources', nDevices, nResources)
    ...
    for (const id in this.accessoryById) {
      if (
        ...
        !this.rtypes.includes(this.deviceById[id].resource.rtype)
      ) {
        this.deleteAccessory(id)
        this.deleteService(id)
        changed = true
    ...
    for (const id in this.deviceById) {
      ...
      this[rtype + 'DeviceByRid'][rid] = device
    }

    for (const rtype of this.rtypes) {
      const rids = Object.keys(this[rtype + 'DeviceByRid']).sort()
      this.debug('%d %s devices', rids.length, rtype)
      for (const rid of rids) {
        const { id } = this[rtype + 'DeviceByRid'][rid]
