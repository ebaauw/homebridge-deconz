...
const ancillaryControlValues = [
  'disarm', 'armed_stay', 'armed_away', 'armed_night',
  'invalid_code', 'panic', 'emergency', 'fire'
]
  ...
  patchLabel (gateway) {
    ...
    switch (this.manufacturer) {
      ...
      case 'THE LIGHT GROUP AS':
        ...
        break
      case 'Universal Electronics Inc':
        switch (this.model) {
          case 'URC4450BC0-X-R':
            buttons.push([1, 'Disarm', SINGLE])
            buttons.push([2, 'Armed Stay', SINGLE])
            buttons.push([3, 'Armed Away', SINGLE])
            buttons.push([4, 'Armed Night', SINGLE])
            buttons.push([5, 'Invalid Code', SINGLE])
            buttons.push([6, 'Panic', SINGLE])
            buttons.push([7, 'Emergency', SINGLE])
            buttons.push([8, 'Fire', SINGLE])
            this.capabilities.toButtonEvent = (v) => {
              return 1000 * 1 + ancillaryControlValues[v]
            }
            break
          default:
            break
